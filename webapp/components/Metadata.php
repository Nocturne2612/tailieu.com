...
class Metadata extends CApplicationComponent {
    ...
    public function getActions($controller, $module = null) {
        ...
        if ($module != null) {
            ...
            $path = 'webapp' . DIRECTORY_SEPARATOR . 'controllers';
        }
        require_once($path . DIRECTORY_SEPARATOR . $controller . '.php');
        ...
        foreach ($methods as $method) {
            if (strpos($method->name, 'action') === 0 and ctype_upper($method->name[6])) {
                $actions[] = mb_convert_case(str_replace('action', '', $method->name), MB_CASE_LOWER, "UTF-8");
            }
        }
        return $actions;
    }
    ...
    public function getModels($module = null, $include_classes = false) {
        ...
        if ($module != null) {
            ...
            $path = 'webapp' . DIRECTORY_SEPARATOR . 'models';
        }
