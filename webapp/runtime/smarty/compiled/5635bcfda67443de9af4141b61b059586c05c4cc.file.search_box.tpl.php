<?php /* Smarty version Smarty-3.1.19, created on 2019-09-09 04:07:41
         compiled from "/home/tailieuk/public_html/webapp/modules/tailieu/components/widgets/views/search_box.tpl" */ ?>
<?php /*%%SmartyHeaderCode:18598989075d63530245e833-46408223%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '5635bcfda67443de9af4141b61b059586c05c4cc' => 
    array (
      0 => '/home/tailieuk/public_html/webapp/modules/tailieu/components/widgets/views/search_box.tpl',
      1 => 1568001976,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '18598989075d63530245e833-46408223',
  'function' => 
  array (
  ),
  'version' => 'Smarty-3.1.19',
  'unifunc' => 'content_5d63530245f053_78585089',
  'has_nocache_code' => false,
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_5d63530245f053_78585089')) {function content_5d63530245f053_78585089($_smarty_tpl) {?><div class="index-banner">
    <div class="index-banner-bg"></div>
    <div class="w1520">
        <div class="index-banner-box">
            <div class="index-night-tips" style="display:none;">

            </div>

            <div class="index-banner-inner">
                <div class="index-banner-main">
                    <a href="/" class="index-banner-logo xs-hide">
                        <img src="/assets/tailieuv3/images/clogo.png" alt="">
                    </a>
                    <p class="index-banner-title">5,254,600 Graphic Resources For Free Download</p>
                    <h1 class="index-banner-text">Royalty Free PNG Images, Vectors, Backgrounds, Templates, Art Fonts</h1>
                    <div class="index-page index-search js_search pullDown ">
                        <div class="index-search-input">
                            <input type="text" placeholder="Search Graphic Design" class="js_search-input " data-type="1" value="">
                        </div>
                        <a href="javascript:;" class="index-search-btn" data-text="Please enter keywords">
                            <i class="iconfont icon-sousuo"></i>
                        </a>

                        <div class="search-dropdown-wrap comment-search-dropdown-wrap" style="display: none;">
                            <div class="search-dropdown"></div>
                        </div>

                    </div>                    <div class="index-banner-recommend xs-hide">
                        <a href="/so/frame" data-type="frame" class="index-banner-recommend-word rem-click">frame</a>
                        <a href="/so/watercolor-flowers" data-type="watercolor flowers" class="index-banner-recommend-word rem-click">watercolor flowers</a>
                        <a href="/so/black-gold" data-type="black gold" class="index-banner-recommend-word rem-click">black gold</a>
                        <a href="/so/smoke" data-type="smoke" class="index-banner-recommend-word rem-click">smoke</a>
                        <a href="/so/sale-label" data-type="sale label" class="index-banner-recommend-word rem-click">sale label</a>
                    </div>
                </div>
                <div class="index-banner-footer xs-hide">
                    <ul class="index-banner-about">
                        <li class="index-banner-about-list">
                            <span class="index-banner-about-icon">
                                <i class="iconfont icon-xiazai"></i>
                            </span>
                            <span class="index-banner-about-text fc-greenLinear">Daily 2 times free downloads</span>
                        </li>
                        <li class="index-banner-about-list">
                            <span class="index-banner-about-icon">
                                <i class="iconfont icon-templates"></i>
                            </span>
                            <span class="index-banner-about-text fc-greenLinear">Super quantity</span>
                        </li>
                        <li class="index-banner-about-list">
                            <span class="index-banner-about-icon">
                                <i class="iconfont icon-clock"></i>
                            </span>
                            <span class="index-banner-about-text fc-greenLinear">Daily updated</span>
                        </li>
                        <li class="index-banner-about-list">
                            <span class="index-banner-about-icon">
                                <i class="iconfont icon-shiyongwendang"></i>
                            </span>
                            <span class="index-banner-about-text fc-greenLinear">PRF Agreement for commercial</span>
                        </li>
                    </ul>
                    <a href="/legal/license-terms" class="index-banner-copy bg-greenLinear">
                        <i class="iconfont icon-banquan"></i>
                        <span>100% Copyright Guaranteed</span>
                    </a>
                </div>
            </div>
        </div>
    </div>
    <div class="index-banner-mask"></div>
</div><?php }} ?>
